...
void Player::keyboardInput(Keyboard &keyboard,const sf::Window &window)
{
    for(int i=0;i<4;i++)
        wasd[i] = 0;
    ...
    if (keyboard.isKeyDown(sf::Keyboard::F1)) {
        if(renderMode == 1)
            renderMode = 3;
        //std::cout << renderMode << std::endl;
    }
    if (keyboard.isKeyDown(sf::Keyboard::F2)) {
        if(renderMode == 3)
            renderMode = 1;
        //std::cout << renderMode << std::endl;
    }
...
void Player::mouseInput(const sf::Window &window)
{
    ...
    rotation.y += change.x * 0.15f;
    rotation.x += change.y * 0.15f;
